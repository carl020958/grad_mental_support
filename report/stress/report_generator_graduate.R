...
par <- 
  par %>% 
  rename(스트레스_해석상담일 = '스트레스 관리\n해석상담일') %>% 
  select(Name, 스트레스_해석상담일) %>% 
  filter(!is.na(스트레스_해석상담일)) %>% 
  mutate(date = lubridate::as_date(stringr::str_sub(스트레스_해석상담일, 1, 10))) %>% 
...
# ---------------------------
drive_list <- 
  googledrive::drive_ls('https://drive.google.com/drive/u/0/folders/1p6VrJy5TtIV5guLiov4WB-PejpoUe2mF')
...
for(i in all_files){
  ...
  googledrive::drive_upload(
    media = i,
    path = googledrive::as_id('https://drive.google.com/drive/u/0/folders/1p6VrJy5TtIV5guLiov4WB-PejpoUe2mF')
  )
}
...
for (i in 1:length(toMove_html_list)){
  ...
  file.copy(
    toMove_html_list[i], 
    '/Users/zsu/Dropbox/JISU/Korea Uni/대학원/Kolab_Projekt/2021/대학원생_심리지원/report/스트레스/html/uploaded')
